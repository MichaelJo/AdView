...
static NSString * const kSDFParseAPIBaseURLString = @"http://aditplanet.com/api/v1/";
...
/*
+ (SDAFParseAPIClient *)sharedClientForClass:(NSString *)className {
    static SDAFParseAPIClient *sharedClient = nil;
    static dispatch_once_t onceToken;
    NSString *urlString = kSDFParseAPIBaseURLString;
    //Promo,Super,Category
    if ([className  isEqual: @"Promo"]) {
            urlString = @"http://aditplanet.com/api/v1/deals/promo";
    }else if ([className  isEqual: @"Super"]) {
            urlString = @"http://aditplanet.com/api/v1/deals/super";
    }else if ([className  isEqual: @"ProductCategory"]) {
        urlString = @"http://aditplanet.com/api/v1/categories/all";
    }else if ([className  isEqual: @"Merchant"]) {
        urlString = @"http://aditplanet.com/api/v1/merchants/all";
    }


    dispatch_once(&onceToken, ^{
        sharedClient = [[SDAFParseAPIClient alloc] initWithBaseURL:[NSURL URLWithString:urlString]];
    });
...
- (NSMutableURLRequest *)GETRequestForClass:(NSString *)className parameters:(NSDictionary *)parameters {
    ...
    NSString *urlString = @"deals/all";
    if ([className  isEqual: @"Promo"]) {
        urlString = @"deals/promo";
    }else if ([className  isEqual: @"Super"]) {
        urlString = @"deals/super";
    }else if ([className  isEqual: @"TopCategory"]) {
        urlString = @"categories/all";
    }else if ([className  isEqual: @"Merchant"]) {
        urlString = @"merchants/all";
    }

    request = [self requestWithMethod: @"GET" path:urlString parameters:parameters];
    return request;
}
