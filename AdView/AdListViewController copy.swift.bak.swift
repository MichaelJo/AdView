...
class AdListViewController: UIViewController, UINavigationBarDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout,ServerManagerDelegate {
    ...
    func NavbarSet(){
       ...
       // {
            Navbar.frame=CGRectMake(0, 0, self.view.frame.size.width, 44+20);
       ...
       // }
        ...
        let imageV = UIImageView(image: BundleImage("top.png"))
        Navbar.addSubview(imageV)
        //[imageV release];
        
        Navbar.backgroundColor=UIColor.whiteColor()
        
        self.view.addSubview(Navbar)
        var title_label = UILabel(frame: CGRect(x: 65, y: Navbar.frame.size.height-44, width: self.view.frame.size.width-130, height: 44)) //[[UILabel alloc]initWithFrame:CGRectMake(65, view_bar.frame.size.height-44, self.view.frame.size.width-130, 44)];
        //    title_label.text=@"首页";
...
//        let backImg = UIImage(named: "BgImage");
...
//        rightButton.addTarget(self, action: "ItemAct:", forControlEvents: UIControlEvents.TouchUpInside);
    ...
    func LayoutSet(){
        ...
        self.view.addConstraints(NSLayoutConstraint.constraintsWithVisualFormat(
            "V:|-0-[Navbar]-[CollectionView]|", options: nil, metrics: nil, views: layout_dict))
        self.view.addConstraints(NSLayoutConstraint.constraintsWithVisualFormat(
