...
-(id)initWithFrameRect:(CGRect)rect scrolArray:(NSArray *)array needTitile:(BOOL)isNeedTitle
{
    ...
    {
        AdProduct *dic = [array objectAtIndex:i];
        [imagePathArray addObject:[dic image]];
        [titleArray addObject:[dic name]];
        [idArray addObject:[dic pid]];
    }
...
//                [imgView setImageWithURL:[NSURL URLWithString:[urlStr stringByAddingPercentEscapesUsingafEncoding:NSUTF8StringEncoding]]];
//            }
            
            NSData * imageData = [[NSData alloc] initWithContentsOfURL: [NSURL URLWithString: [[array objectAtIndex:i] image]]];
            ...
            [Tap setAccessibilityHint:[[array objectAtIndex:i] pid]];
...
- (void)imagePressed:(UITapGestureRecognizer *)sender
{
    ...
    //[app.navigationController pushViewController:mvc animated:YES];
