...
//    _array=[[NSMutableArray alloc]initWithObjects:@"邀  请  码:",@"用  户  名:",@"密       码:",@"确认密码:", nil];
    _array=[[NSMutableArray alloc]initWithObjects:@"用  户  名:",@"密       码:",@"确认密码:", nil];
}
-(void)addImage
{
    ...
    NSArray*arrayTitle=[[NSArray alloc]initWithObjects:@"6-16个字符",@"输入密码",@"再次输入密码", nil];
...
//    }
        ...
        CoreDataBase *cbd = [[CoreDataBase alloc] init];
        [cbd register:field1.text password:field2.text email:@"notset@22.net"];
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:@"userCreated" object:nil queue:nil usingBlock:^(NSNotification *note) {
            NSLog(@"ff:%@",[note.userInfo objectForKey:@"user_name"]);
            [[NSNotificationCenter defaultCenter] removeObserver:self name:@"userCreatingFailed" object:nil];
            [[NSNotificationCenter defaultCenter] removeObserver:self name:@"userCreated" object:nil];

        }];
        //[cbd register:field1.text ];
