...
- (void)viewDidLoad
{
    ...
    if (![default1 objectForKey:@"applicationVersion"]||![[default1 objectForKey:@"applicationVersion"] isEqualToString:@"0.1"]) {
    
        [self presentViewController:[[[GuideViewController alloc] init] autorelease] animated:NO completion:nil];
        [default1 setObject:@"0.1" forKey:@"applicationVersion"];
...
-(void)loadSpinner{
    UIActivityIndicatorView *spinner = [[UIActivityIndicatorView alloc]initWithFrame:CGRectMake(105,140,110,110)];
    ...
    }else if([self isUserName:fieldPsw.text ]==NO)
    {
        ...
        dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
            ...
            dispatch_sync(dispatch_get_main_queue(), ^{
                ...
                appDel.navigationController=[[LTKNavigationViewController alloc]initWithRootViewController:view1];
...
-(void)loadMainView
{
    ...
    UserAccountSetting* ua = [[UserAccountSetting alloc] init];
    [ua UserListDataAdd:fieldName.text account:fieldName.text password:fieldPsw.text];
    //[ua UserListDataAdd:fieldName.text,fieldName.text,fieldPsw.text];
...
-(void)btnSJ:(id)sender
{
    [fieldName resignFirstResponder];
    [fieldPsw resignFirstResponder];
    //FCRsgisterViewController *upgrade=[[FCRsgisterViewController alloc]init];
    
    RegViewController *upgrade=[[RegViewController alloc]init];
