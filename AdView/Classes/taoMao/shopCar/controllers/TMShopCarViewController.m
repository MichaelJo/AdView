...
const NSUInteger TABLEVIEW_OFFSET = 80;
...
-(void)listLoaded:(NSString *)error items:(NSArray *)items
{
    if(error == nil)
    {
        [self addViews];
...
-(void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:@"InsertedOrUpdatedAnDeal" object:nil queue:nil usingBlock:^(NSNotification *note) {
        [self addViews];
        [self initNavigationBar];
    }];
...
- (void)loadData{
    ...
    [self loadRecordsFromAPI];
    
}
- (void)loadRecordsFromAPI {
    ShoppingCartManager *manager = [[ShoppingCartManager alloc] init];
...
-(void)addViews
{
    ...
    if (_isTabbar==YES)
    {
        _tableView =[[UITableView alloc]initWithFrame:CGRectMake(0, TABLEVIEW_OFFSET, self.view.frame.size.width, self.view.frame.size.height-TABLEVIEW_OFFSET-99)style:UITableViewStylePlain];
    }else{
        
        _tableView =[[UITableView alloc]initWithFrame:CGRectMake(0, TABLEVIEW_OFFSET, self.view.frame.size.width, self.view.frame.size.height-TABLEVIEW_OFFSET-50)style:UITableViewStylePlain];
    }
...
-(UIView*)initNavigationBar
{
    ...
    checkBtn1.frame=CGRectMake(8, height_offset, 40, 30);
    ...
    [checkBtn1 setTitleColor:titleColor forState:0];
    [checkBtn1 addTarget:self action:@selector(deleteBtn:) forControlEvents:UIControlEventTouchUpInside];
    [view_bar addSubview:checkBtn1];
    /*
     UIButton *checkBtnAll=[UIButton buttonWithType:0];
     checkBtnAll.frame=CGRectMake(checkBtn1.frame.size.width+checkBtn1.frame.origin.x+10, height_offset, 40, 30);
     ...
     [checkBtnAll setTitleColor:titleColor forState:0];
     [checkBtnAll addTarget:self action:@selector(btnSelecdAll:) forControlEvents:UIControlEventTouchUpInside];
     [view_bar addSubview:checkBtnAll];
     */
    UILabel *title_label=[[UILabel alloc]initWithFrame:CGRectMake(checkBtn1.frame.size.width+checkBtn1.frame.origin.x+5, height_offset - 7, 82, 20)];
    title_label.text=@"购物车合计:";
    title_label.font=[UIFont boldSystemFontOfSize:14];
    title_label.backgroundColor=backgroundColor;
    title_label.textColor =titleColor;
    title_label.textAlignment=0;
    [view_bar addSubview:title_label];
    
    AdPreferences *prefs = [AdPreferences sharedInstance];
    
    UILabel *price_label=[[UILabel alloc]initWithFrame:CGRectMake(title_label.frame.size.width+title_label.frame.origin.x, height_offset - 7, 75, 20)];
    price_label.tag=-101;
    price_label.text=[NSString stringWithFormat:@"%d",prefs.cartGrandTotal];
    price_label.font=[UIFont boldSystemFontOfSize:14];
    price_label.backgroundColor=backgroundColor;
    price_label.textColor =titleColor;
    price_label.textAlignment=0;
    [view_bar addSubview:price_label];
    
    
    UILabel *my_points_title_label=[[UILabel alloc]initWithFrame:CGRectMake(checkBtn1.frame.size.width+checkBtn1.frame.origin.x+5, height_offset+12, 82, 20)];
    my_points_title_label.text=@"我的积分:";
    my_points_title_label.font=[UIFont boldSystemFontOfSize:14];
    my_points_title_label.backgroundColor=backgroundColor;
    my_points_title_label.textColor =titleColor;
    my_points_title_label.textAlignment=0;
    [view_bar addSubview:my_points_title_label];
    
    //AdPreferences *prefs = [AdPreferences sharedInstance];
    
    UILabel *points_label=[[UILabel alloc]initWithFrame:CGRectMake(title_label.frame.size.width+my_points_title_label.frame.origin.x, height_offset+12, 75, 20)];
    points_label.tag=-501;
    points_label.text=[NSString stringWithFormat:@"%d",prefs.cartGrandTotal];
    points_label.font=[UIFont boldSystemFontOfSize:14];
    points_label.backgroundColor=backgroundColor;
    points_label.textColor =titleColor;
    points_label.textAlignment=0;
    [view_bar addSubview:points_label];

    
    UIButton *acountBtn1=[UIButton buttonWithType:0];
    acountBtn1.frame=CGRectMake(view_bar.frame.size.width-95, height_offset, 85, 30);
...
{
    AdPreferences *prefs = [AdPreferences sharedInstance];
    //prefs.cartItems = items;
    return [prefs.cartItemIdArray count];
}
...
- (FCShoppingCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.isTouch=NO;
    NSString *c = _mak;
    if([prefs.cartItemIdArray count] == 0)
        return cell;
    ...
    AdProduct *product = [cdb getOne: [prefs.cartItemDealIdArray objectAtIndex:rowIndex]];
    ...
    [btnCut setTitle:[prefs.cartItemIdArray objectAtIndex:rowIndex] forState:UIControlStateNormal];
    ...
    NSString *quantity = [NSString stringWithFormat:@"%@",[prefs.cartItemQuantityArray objectAtIndex:rowIndex]];
    ...
    [btnAdd setTitle:[prefs.cartItemIdArray objectAtIndex:rowIndex] forState:UIControlStateNormal];
    ...
    //
    //        }
    [imgView setImageWithURL:[NSURL URLWithString: [product image]]];
    ...
    title.text= [[_marrayAll objectAtIndex:0]name];
    ...
    //颜色
    UILabel*labelColor=[[UILabel alloc]initWithFrame:CGRectMake(title.frame.origin.x, title.frame.origin.y+title.frame.size.height, 205, 20)];
    ...
    NSLog([NSString stringWithFormat:@"%d",indexPath.row]);//    indexPath.row;
    
    labelColor.text=[NSString stringWithFormat:@"%@",[product name]];
    //CartItem * cartitem =[_marrayAll objectAtIndex:indexPath.row];
    NSString *bb = [prefs.cartItemDealIdArray objectAtIndex:rowIndex];
    ...
    labelRuler.text=[NSString stringWithFormat:@":%@",bb];
    ...
    */
    ...
    //        NSArray *arr=[NSArray arrayWithArray:_arrayPrice];
    labelPrice1.text=[NSString stringWithFormat:@"%@",[prefs.cartItemAdpointsArray objectAtIndex:indexPath.row]];
    ...
    //        NSArray *arr=[NSArray arrayWithArray:_arrayPrice];
    labelPrice4.text=[NSString stringWithFormat:@"%0.f",[[prefs.cartItemAdpointsArray objectAtIndex:indexPath.row] floatValue]*[[prefs.cartItemQuantityArray objectAtIndex:indexPath.row] floatValue] ];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell.isTouch==NO)
    {
        ...
        if(![deleteArray containsObject:path]){
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    for (int i=0; i<[_marrayAll count]; i++)
    {
        ...
        if (textField.tag==field.tag)
        {
            ...
            label.text= [NSString stringWithFormat:@"%.0f",[[prefs.cartItemAdpointsArray objectAtIndex:i] floatValue]*[field.text floatValue]];
...
//    }
//}
-(void)deleteAdPreferenceItem: (int)rowIndex
{
    AdPreferences *prefs = [AdPreferences sharedInstance];
    prefs.cartItemDealIdArray = [self deleteItem:prefs.cartItemDealIdArray at:rowIndex];
    prefs.cartItemIdArray = [self deleteItem:prefs.cartItemIdArray at:rowIndex];
    prefs.cartItemQuantityArray = [self deleteItem:prefs.cartItemQuantityArray at:rowIndex];
    prefs.cartItemAdpointsArray = [self deleteItem:prefs.cartItemAdpointsArray at:rowIndex];
...
-(NSArray*)deleteItem: (NSArray*)orignal at:(int)rowIndex
{
    NSMutableArray *tempArray = [NSMutableArray arrayWithArray:orignal];
    
    [tempArray removeObjectAtIndex:rowIndex];
...
-(void)deleteColection
{
    for(NSIndexPath *path in deleteArray)
    {
        ...
        ShoppingCartManager *manager = [[ShoppingCartManager alloc] init];
        manager.delegate = self;
        NSLog([NSString stringWithFormat:@"%d,%d",[prefs.cartItemDealIdArray count],path.row]);
        [manager deleteItem: [prefs.cartItemDealIdArray objectAtIndex:path.row]];
        
        //prefs.cartItems = items;
        [self deleteAdPreferenceItem:path.row];
        
        //[prefs.cartItemIdArray
        
        UITableView* tableView=(UITableView*)[self.view viewWithTag:7777777];
...
#pragma mark多选
-(void)updateQuantity: (NSInteger)tag newValue:(NSString*)newValue
{
    AdPreferences *prefs = [AdPreferences sharedInstance];
    //prefs.cartItems = items;
    //return [prefs.cartItemId count];
    NSMutableArray *tempArray = [NSMutableArray arrayWithArray:prefs.cartItemQuantityArray];
    
    [tempArray replaceObjectAtIndex:tag withObject:newValue];
    prefs.cartItemQuantityArray = [NSArray arrayWithArray: tempArray];
    
    //[prefs.cartItemQuantityArray replaceObjectAtIndex:tag withObject:newValue];
    ShoppingCartManager *manager = [[ShoppingCartManager alloc] init];
    manager.delegate = self;
    [manager updateItemQuantity:[prefs.cartItemDealIdArray objectAtIndex:tag] newValue:newValue];
    //[self updatePrice];
    float _p = 0;
    for (int i =0; i<prefs.cartItemIdArray.count; i++)
    {
        _p=[[prefs.cartItemQuantityArray objectAtIndex:i] floatValue] * [[prefs.cartItemAdpointsArray objectAtIndex:i] floatValue] +_p;
        //   label1.text= [NSString stringWithFormat:@"%.0f", [[btn titleForState:UIControlStateNormal]  floatValue]*[field.text floatValue]];
    }
    //UILabel*label1=(UILabel*)[self.view viewWithTag:11000000+tag];
    
    //label1.text= [NSString stringWithFormat:@"%.0f", [[prefs.cartItemAdpointsArray objectAtIndex:tag] floatValue]*[newValue floatValue]];
    prefs.cartGrandTotal = (int)_p;
    [self updateSubTotal:tag adpoints:[prefs.cartItemAdpointsArray objectAtIndex:tag] newValue:newValue];
    [self updateTotal: prefs.cartGrandTotal];
}
-(void) updateTotal: (NSInteger)total {
    UILabel *labelPriceSum=(UILabel*)[self.view viewWithTag:-101];
    labelPriceSum.text=[NSString stringWithFormat:@"%d",total];
    
}
-(void) updateSubTotal: (NSInteger)tag adpoints:(NSString*)adpoints newValue:(NSString*)newValue
{
    UILabel*label1=(UILabel*)[self.view viewWithTag:11000000+tag];
    
    label1.text= [NSString stringWithFormat:@"%.0f", [adpoints floatValue]*[newValue floatValue]];
}
-(void)btnProcess:(id)sender increase:(BOOL)increase{
    UIButton*btn=(UIButton*)sender;
    ...
    [self updateQuantity:btn.tag newValue:field.text];
...
{
    [self btnProcess:sender increase:false];
}
...
{
    //AdPreferences *prefs = [AdPreferences sharedInstance];
    [self btnProcess:sender increase:true];
    /*
     UIButton*btn=(UIButton*)sender;
     UITextField*field=(UITextField* )[self.view viewWithTag:100000+btn.tag ];
     ...
     label1.text= [NSString stringWithFormat:@"%.0f", [[prefs.cartItemAdpointsArray objectAtIndex:btn.tag] floatValue]*[field.text floatValue]];
     NSLog(@"%@",deleteArray);
