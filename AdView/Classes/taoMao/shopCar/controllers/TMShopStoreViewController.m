...
-(void)viewDidLoad
{
    MerchantManager *cdb = [[MerchantManager alloc] init];
    merchants = [cdb getList];
    [super viewDidLoad];
    self.navigationController.navigationBarHidden=YES;
    self.view.backgroundColor = [UIColor whiteColor];
    for (int i =0; i < [merchants count]; i++) {
        NSLog([NSString stringWithFormat:@"%@",[[merchants objectAtIndex:i] name]]);
    }
...
//        [cell addSubview:bgImage];
        ...
        //NSData * imageData = [[NSData alloc] initWithContentsOfURL: [NSURL URLWithString: [[merchants objectAtIndex:indexPath.row] imageUrl]]];
        
//        [btn setImage:[UIImage imageWithData:imageData] forState:0];
        NSString * imageUrl = [[merchants objectAtIndex:indexPath.row] imageUrl];
        [headImg setImageWithURL:[NSURL URLWithString: imageUrl]];
        //headImg.image=[UIImage imageWithData:imageData];//BundleImage(@"df_03_.png");
...
//        [cell addSubview:imgView];
        AdMerchant* mer = [merchants objectAtIndex:indexPath.row];
        cell.accessibilityHint = mer.id;
        //title店铺名
        RTLabel*title=[[RTLabel alloc]initWithFrame:CGRectMake(imgView.frame.size.width+imgView.frame.origin.y+20, 25, 100, 20)];
        title.backgroundColor=[UIColor clearColor];
        title.textAlignment=0;
        //title.text=[[merchants objectAtIndex:indexPath.row] name];
    [title setText:[[merchants objectAtIndex:indexPath.row] name]];
        title.textColor=[UIColor colorWithRed:.2 green:.2 blue:.2 alpha:1.0];
        title.font=[UIFont systemFontOfSize:14];
        [cell addSubview:title];
        
        //店铺介绍
        RTLabel*title1=[[RTLabel alloc]initWithFrame:CGRectMake(title.frame.origin.x, title.frame.origin.y+title.frame.size.height+5, 200, 40)];
        ...
        title1.text=[[merchants objectAtIndex:indexPath.row] sub_description];
        title1.backgroundColor=[UIColor clearColor];
        [cell addSubview:title1];
        
        //店铺介绍详情
        /*UILabel*title2=[[UILabel alloc]initWithFrame:CGRectMake(title1.frame.origin.x, title1.frame.origin.y+title1.frame.size.height+5, 200, 20)];
        ...
        title2.text=[[merchants objectAtIndex:indexPath.row] sub_description];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TMThirdClassViewController *list =[[TMThirdClassViewController alloc]initWithTopbar:false];
    UITableViewCell *cell = [self tableView:tableView cellForRowAtIndexPath:indexPath];

    list.merchantId = cell.accessibilityHint;
    TMAppDelegate *delegate=(TMAppDelegate*)[UIApplication sharedApplication].delegate;
